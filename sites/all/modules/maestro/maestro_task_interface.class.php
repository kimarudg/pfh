...
abstract class MaestroTaskInterface {
  ...
  function destroy() {
    ...
    if ((array_key_exists('confirm_delete', $_POST) && $_POST['confirm_delete'] == 1) || $rec == '') {
      ...
      $retval .= "enable_ajax_indicator(); (function($) {\$.ajax({
        type: 'POST',
        url: ajax_url + 'MaestroTaskInterface{$this->_task_type}/{$this->_task_id}/0/destroy/{$this->_security_token}',
        cache: false,
    ...
    return array('message' => $retval, 'success' => $success, 'task_id' => $this->_task_id, 'token' => $this->_security_token );
  }
  ...
  function getAssignmentDisplay() {
    ...
    if (module_exists('og')) {
      ...
      $query->leftJoin($og_values['table'], 'e', 'a.assign_id=e.' . $og_values['gid_column']);
      $query->addField('e', $og_values['title_column'], 'groupname');
    }
